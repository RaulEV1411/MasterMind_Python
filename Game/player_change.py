...
class Game_select:
    ...
    def dar_pistas(self,combinacion_user,combinacion_secreta,turno):
        array_ayudas = [[] for _ in range(4)]
        correctas = f"🟢"
        print(correctas)
        for i in range(4):
            if combinacion_user[i] == combinacion_secreta[i]:
                array_ayudas[i].append(correctas)
                
            else:
                array_ayudas[i].append("🔴")
        print(array_ayudas)
        tablero.añadir_al_array(combinacion_user,turno,sorted(array_ayudas, reverse=True))
        return
